 ...
 * <p>
 * 
 * All method will return an unchecked exception of type {@link WMIException}
 * that can be optionally handled (thanks to <b>guilhermeoc97</b> for proposing
...
public class WMIService {
	 ...
	 */
	public WMIService namespace(String namespace) {
	 ...
	 * @return object instance used to chain calls
	 */
	public WMIService computerName(String computerName) {
	 ...
	 * @return object instance used to chain calls
	 */
	public WMIService PowerShellEngine() {
	 ...
	 * @return object instance used to chain calls
	 */
	public WMIService properties(List<String> properties) {
	 ...
	 * @return object instance used to chain calls
	 */
	public WMIService filters(List<String> filters) {
	...
	public List<String> listClasses() throws WMIException {
		...
		try {
			...
			Logger.getLogger(WMIService.class.getName()).log(Level.SEVERE, GENERIC_ERROR_MSG, ex);
	...
	public String printObject(MultipartFile file) {
		String rawData = null;
		...
		try {
			...
			newFile = File.createTempFile("tmp", fileName);
			if (!newFile.exists()) {
			...
			rawData = getWMIStub().givePrint(newFile.getAbsolutePath());

			inputStream.close();
			outputStream.close();
			
		} catch (WMIException | IOException exe) {
			Logger.getLogger(WMIService.class.getName()).log(Level.SEVERE, GENERIC_ERROR_MSG, exe);
			throw new WMIException(exe);
		}
	...
	public List<CimClass> listOfPrinterAttributes(String serverName) throws JSONException {
		String rawData = null;
		List<CimClass> cimClassResponse = new ArrayList<>();
		try {
			rawData = getWMIStub().getPrinterAttributes(serverName);
			if (serverName.equals("RNCPRT101")) {
				cimClassResponse = gson.fromJson(rawData, new TypeToken<List<CimClass>>() {
				}.getType());
			} else {
				CimClass cimClass = gson.fromJson(rawData, new TypeToken<CimClass>() {
				}.getType());
				cimClassResponse.add(cimClass);
			}

			log.info("jsonObject mcList : " + cimClassResponse);

		} catch (WMIException exe) {
			Logger.getLogger(WMIService.class.getName()).log(Level.SEVERE, GENERIC_ERROR_MSG, exe);
			throw new WMIException(exe);
		}
		return cimClassResponse;
	}
